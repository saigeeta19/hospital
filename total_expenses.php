...
$("#view").click(function(){
	...
	var category=$("#category").val();
                     ...
                     $.ajax({
                        type:"post",
                        url:"getexpensereports.php",
                        data:"fromdate="+fromdate +"&todate="+todate+"&entry="+entry+"&category="+category,
...
<form name="patient_register" method="post" >
     <tr>
            <td colspan="5"><p id="panel">Total Expenses</p></td>
            </tr>
        <table id="table11"  name="t1" border="2" width="100%" >
            <tr>
			   ...
			   <td id="headus">Category</td>
               <td><select name="category" id="category">
                <option value="all">All</option>
                <?php
                $sql=mysqli_query($con,"SELECT * FROM expense_categories");
                while($sql1=mysqli_fetch_array($sql))
                {
                    echo "<option value='".$sql1['expense_category']."'>$sql1[expense_category]</option>";
                }
                ?>
                </select>
               </td>
