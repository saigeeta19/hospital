...
for ($i=$fdate; $i<=$tdate; $i+=86400) {
       $cdate=date("d-m-Y", $i);
       $sql21=mysqli_query($con,"SELECT sum(discount_amount) FROM procedures_discounts WHERE date LIKE '$cdate%'");
				$sql31=mysqli_fetch_array($sql21);
				$amount=$sql31['sum(discount_amount)'];
                                $discountamt=$discountamt+$amount;
       $query=mysqli_query("$con,SELECT * FROM procedure_entry WHERE date LIKE '$cdate%' GROUP BY bill_number ORDER BY id DESC");
       while($row=mysqli_fetch_array($query))
       {
				...
				$amount=$sql3['sum(procedure_fees)'];
                $uid=$row['patient_id'];
                ...
                if($row['status']=="pro")
                {
                 ...
                 $ghd=  mysqli_query($con,"SELECT * FROM due_amounts WHERE bill_number='".$row[bill_number]."' && opd_type='pro'");
