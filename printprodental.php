...
$entry_person=$entry1['name'];
 ...
 $query=mysqli_query($con,"SELECT * FROM procedure_dental_entry WHERE bill_number='".$bill."'");
 $row=mysqli_fetch_array($query);
	 ...
	 $gender=$row['gender'];
      ...
      $procedure_fees=$row['dental_fees'];
     
     $date=$row['date'];
     $date = date("d-M-Y", strtotime($date));
   	 $uid=$patient_id;
                $sql45=mysqli_query($con,"SELECT * FROM patients WHERE uid='".$uid."'",$con);
                $sql15=mysqli_fetch_array($sql45);
                $patient_name=$sql15['name'];
                
                  $query23=mysqli_query($con,"SELECT * FROM procedures_dental_discounts WHERE bill_number='".$bill."'");
    ...
    $t="<html><table width='75%' style='font-size:14px ;padding-top:100px;padding-left:90px;line-height:30px;line-width:15px;'>
       ...
       <tr>
	    ...
	    ";
	    $query1=mysqli_query($con,"SELECT * FROM procedure_dental_entry WHERE bill_number='".$bill."'");
	  while($row1=mysqli_fetch_array($query1))
		{
	    $pro1=$row1['dental_name'];
	    $amt=$row1['dental_fees'];
			$t=$t."<tr>
			<td>$inc</td>
			<td>$pro1</td>
			<td>Rs. $amt</td>
