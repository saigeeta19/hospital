...
?>
    <?php include 'header.php'; ?>
     ...
     <?php
       ...
       if($entry=="all")
       {
     $sql = "SELECT count(id) FROM pharmacy_entry WHERE date LIKE '$cdate%' && type='".$ptype."' GROUP BY bill_number ";
       }
       else
       {
      $sql="SELECT count(id) FROM pharmacy_entry WHERE date LIKE '$cdate%' && pid='".$entry."' && type='".$ptype."' GROUP BY bill_number"  ;  
       }
        ...
        // $rec_count = $row[0];
         ...
         if($entry=="all")
         {
         $sql = "SELECT * FROM pharmacy_entry WHERE date LIKE '$cdate%' && type='".$ptype."' GROUP BY bill_number LIMIT $offset, $rec_limit";
         }
         else
         {
              $sql="SELECT * FROM pharmacy_entry WHERE date LIKE '$cdate%' && pid='".$entry."' && type='".$ptype."' GROUP BY bill_number LIMIT $offset, $rec_limit"  ;  
         }
         $retval = mysqli_query( $sql, $con );
...
<form name="opd_list" method="post">
    <tr>
            <td colspan="5"><p id="panel">Pharmacy Cash Collection Detail Dashboard</p></td>
            </tr>
    <tr >
                <td colspan="5">
            <p align="right">
