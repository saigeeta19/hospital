...
for ($i=$fdate; $i<=$tdate; $i+=86400) {
       ...
       $query=mysqli_query($con,"SELECT * FROM procedure_dental_entry WHERE date LIKE '$cdate%' GROUP BY bill_number ORDER BY id DESC");
       while($row=mysqli_fetch_array($query))
       {
                ...
                if($row['status']=="pro")
                {
                 ...
                 $ghd=  mysqli_query($con,"SELECT * FROM due_amounts WHERE bill_number='".$row[bill_number]."' && opd_type='den'");
                ...
                {
                    $d="<br/>(Due)";
                     $k="<br/><a href='changestatus.php?type=dpro&bill=".$row['bill_number']."'><input type='button' value='Change' /></a>";
                    $dueamount=$dueamount+$amount;
                }
                else
                {
