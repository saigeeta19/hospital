...
for ($i=$fdate; $i<=$tdate; $i+=86400) {
       $cdate=date("d-m-Y", $i);
       $query=mysqli_query($con,"SELECT * FROM patient_admission WHERE admission_date LIKE '$cdate%'");
       while($row=mysqli_fetch_array($query))
       {
             $adate=$row['admission_date'];
                $adate = date(" d-M-Y h:i a", strtotime($adate));
				$ldate=$row['leaving_date'];
				if($ldate=="")
				{
					$ldate="";
				}
				else {
					$ldate = date(" d-M-Y h:i a", strtotime($ldate));
				}
           ...
           <tr>
             ...
             <td align='center'>$row[category] / $row[ward_name] / $row[bed_room_name]</td>
             <td align='center'>$ldate</td>
