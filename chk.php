...
function thours($start,$end)
{
 ...
 ?>
  ...
  <table id="tablepaging" class="yui" name="t1" border="1" width="100%" style="border-collapse: collapse;" >
            
            <tr>
            	<td colspan="10"><h1 style="text-align: center;">Today's Payable (<?php echo date("d-m-Y"); ?>)</h1></td>
            </tr>
            <tr>
                <th>S.No.</th>
                <th>UID</th>
...
echo "<input type='hidden' name='emergency' value='".$emer_charge."' />";
 ...
 while($row=mysqli_fetch_array($query))
 {
   ...
   $h=floor($h);
    // $chk=ceil($h/24);
     $query1= mysqli_query($con,"SELECT * FROM wards WHERE category='".$category."' && ward_name='".$ward_name."'",$con);
