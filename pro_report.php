...
$("#view").click(function(){
    ...
    var pro_name=$("#pro_name").val();
                     ...
                     $.ajax({
                        type:"post",
                        url:"getmarproreports.php",
                        data:"fromdate="+fromdate +"&todate="+todate+"&pro_name="+pro_name,
...
<form name="patient_register" method="post" >
     <tr>
            <td colspan="5"><p id="panel">Total Patient Details & PRO Report</p></td>
            </tr>
        <table id="table11"  name="t1" border="2" width="100%" >
            <tr>
               ...
               <td id="headus">PRO</td>
               <td><select name="pro_name" id="pro_name" required="required">
                    <option value="all">All</option>
                    <?php
                    $query=mysqli_query($con,"SELECT * FROM marketing_prolist");
                    while($row=mysqli_fetch_array($query))
                    {
                        echo "<option value='".$row['pro_name']."'>$row[pro_name]</option>";
                    }
                    ?>
