...
var html = <?php echo json_encode($tr); ?>;
 ...
 var printWin = window.open("","","left=0,top=0,width=1,height=1,toolbar=0,scrollbars=0,status  =0");
  ...
  //  {
        ...
        {
	$query=mysqli_query($con,"INSERT INTO deposits(patient_ip_id,patient_id,date,bill_number,amount,mode,status,entry_person) VALUES ('".$patient_ip_id."','".$patient_id."','".$date."','".$bill_number."','".$amount."','".$mode."','D','".$entry_person."')",$con);
        }
    if($query)
	{
...
<form name="deposit" method="post" onsubmit="return confirm('Are you sure?');">
		<tr>
            <td colspan="4"><p id="panel">Deposit Entry Page</p></td>
            </tr>
