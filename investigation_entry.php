...
<form name="opd_entry" method="post" >
	<tr>
            <td colspan="4"><p id="panel">Investigation Billing</p></td>
            </tr>	
			...
			<tr>
				...
				<td><input type="radio" name="gender" id="male" value="Male" />Male<input type="radio" name="gender" id="female" value="Female" />Female</td>
                        ...
                        <tr>
                            ...
                            <td><select name="credit_company">
							<option value="0">No Company</option>
							<?php
							$as=mysqli_query($con,"SELECT * FROM credit_company_list order by company_name");
							while($arow=mysqli_fetch_array($as))
							{
								echo "<option value='".$arow['id']."'>".$arow['company_name']."</option>";
							}
							?>
							</select></td>
                        </tr>
...
if($_POST['submit'])
{
	...
	// For Bill number
	   ...
	   echo "helloneharika";
	   echo $f="INSERT INTO investigation_entry(bill_number,date,patient_id,age,gender,investigation,investigation_fees,mode,status,due_status,due_reason,entry_person,company_name) VALUES ('".$bill_number."','".$date."','".$patient_id."','".$age."','".$gender."','".$inv."','".$amt."','".$pmode."','inv','".$due_status."','".$due_reason."','".$entry_person."','".$credit_company."')";
	   
       //Bill number ends here
	foreach($_REQUEST['inv_list'] as $inv)
	{
		
		$query1=mysqli_query($con,"SELECT investigation_amount FROM investigations_list WHERE investigation_name='".$inv."'");
	    $row1=mysqli_fetch_array($query1);
	    $amt=$row1['investigation_amount'];
            if($pmode=="Card")
        {
                $query=mysqli_query($con,"INSERT INTO investigation_entry(bill_number,date,patient_id,age,gender,investigation,investigation_fees,`mode`, `bank_name`, `holder_name`, `cheque_number`,status,due_status,due_reason,entry_person,company_name) VALUES ('".$bill_number."','".$date."','".$patient_id."','".$age."','".$gender."','".$inv."','".$amt."','".$pmode."','".$card_bank_name."','".$card_holder."','".$cheque_number."','inv','".$due_status."','".$due_reason."','".$entry_person."','".$credit_company."')");
          
        }
        else if($pmode=="Cheque")
        {
            $query=mysqli_query($con,"INSERT INTO investigation_entry(bill_number,date,patient_id,age,gender,investigation,investigation_fees,`mode`, `bank_name`, `holder_name`, `cheque_number`,status,due_status,due_reason,entry_person,company_name) VALUES ('".$bill_number."','".$date."','".$patient_id."','".$age."','".$gender."','".$inv."','".$amt."','".$pmode."','".$cheque_bank_name."','".$cheque_holder."','".$cheque_number."','inv','".$due_status."','".$due_reason."','".$entry_person."','".$credit_company."')");
        }  
        else if($pmode=="Cash")
        {
            $query=mysqli_query($con,"INSERT INTO investigation_entry(bill_number,date,patient_id,age,gender,investigation,investigation_fees,mode,status,due_status,due_reason,entry_person,company_name) VALUES ('".$bill_number."','".$date."','".$patient_id."','".$age."','".$gender."','".$inv."','".$amt."','".$pmode."','inv','".$due_status."','".$due_reason."','".$entry_person."','".$credit_company."')");
			
        }
		//$query=mysqli_query($con,"INSERT INTO investigation_entry(bill_number,date,patient_id,age,gender,investigation,investigation_fees,status,entry_person) VALUES ('".$bill_number."','".$date."','".$patient_id."','".$age."','".$gender."','".$inv."','".$amt."','inv','".$entry_person."')");
   ...
   $uid=$patient_id;
     ...
     $query777=  mysqli_query($con,"INSERT INTO `investigations_discounts`(`bill_number`, `discount_mode`, `discount_value`, `discount_amount`,discount_reason,date) VALUES ('".$bill_number."','".$pdmode."','".$pdval."','".$damt."','".$dis_reason."','".$date."')");
	$inc=1; $tot=0;
   $date = date("d-M-Y", strtotime($date));
	...
	$t="<html><table width='75%' style='font-size:14px ;padding-top:100px;padding-left:90px;line-height:30px;line-width:15px;'>
       ...
       <tr>
		...
		{
			$query1=mysqli_query($con,"SELECT investigation_amount FROM investigations_list WHERE investigation_name='".$inv1."'");
	    $row1=mysqli_fetch_array($query1);
	    $amt=$row1['investigation_amount'];
			$t=$t."<tr>
			<td>$inc</td>
...
var html = <?php echo json_encode($t); ?>;
 ...
 <?php  
    ...
    if($query)
    {
       ...
       echo "<script>alert('Investigation Entry successfull');printPage();</script>";
