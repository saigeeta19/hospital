...
function thours($start,$end)
{
 ...
 <form name="opd_list" method="post">
    <tr>
            <td colspan="5"><p id="panel">Promisory Note Pending Patients</p></td>
            </tr>
    <tr>
                <td colspan="5">
            <p align="right">
...
<tr>
 ...
 <?php
 $inc=1;
 $sql56=mysqli_query($con,"SELECT * FROM patient_diagnosis WHERE status='discharged'");
...
$query8=mysqli_query($con,"SELECT sum(amount) FROM deposits WHERE patient_ip_id='".$patient_ip_id."' && status='FSREF'");
 ...
 $totalbill1=$emer_charge+$regcharge+$housekeeping_charge+$trent+$otheramount+$investigation_amount+$ot_entry+$procedure_amount+$equipments_amount+$cons+$consultation_amount;
 $totalbill=($emer_charge+$regcharge+$housekeeping_charge+$otheramount+$trent+$investigation_amount+$ot_entry+$procedure_amount+$equipments_amount+$cons+$consultation_amount)-$discounts;
